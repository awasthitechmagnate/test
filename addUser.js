...
const UserFormModal = () => {
  ...
  //  const [bing_projects, setbing_Projects] = useState([]); // it will appear in mongodb
    const [bingprojects, setBingProjects] = useState([]);
     const [youtubeprojects, setYoutubeProjects] = useState([]);
                ...
                const [llmprojects, setLlmProjects] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await getProjects();
      const allProjects = response.data.projects;
      console.log(allProjects,"god of war")
      // setProjects(allProjects);
      const projectOptions = allProjects.map((item) => ({
        value: item._id,
        label: item.project_name,
  ...
  // ----------------------------------------------------------------------------------------------
   ...
   // ----------------------------getlocalranking--------------------------------------------
    ...
    useEffect(() => {
    async function fetchData() {
      const response = await getLLMProjects();
      console.log(response.data.projects, "awasthi ai localproject")
      const allProjects = response.data.projects;
      // setProjects(allProjects);
      const projectOptions = allProjects.map((item) => ({
...
{/* ---------------------------------LLM------------------------------------ */}
 ...
 <div className="col-12">
                ...
                <ProjectsMultiSelect
                  options={llmprojects}
                  selectedValues={formData.llm_projects}
                  onChange={(vals) =>
                    setFormData((prev) => ({
                      ...prev,
                      llm_projects: vals,
